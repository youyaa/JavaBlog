...
![exchange](img/RabbitMQ-exchange.jpeg)
...
MQ节点会回调函数，告诉我们消息是投递成功，还是失败。

若返回失败，则可以根据需要记录错误日志，或者重发。
...
继承DefaultConsumer类，重写handleDelivery方法
...
1. 关闭自动提交
   ...
   ```java
   channel.basicConsume(QUEUE_NAME, false, new MyConsumer());
   ```
